...
def search():
    ...
    try:
        with yt_dlp.YoutubeDL(SEARCH_OPTS) as ydl:
            # Switched to SoundCloud search to bypass datacenter IP blocks!
...
def refresh_url():
    ...
    cached = get_cached_url(track_id)
    if cached:
        return jsonify({"success": True, "data": {"url": cached}})

    try:
        with yt_dlp.YoutubeDL(REFRESH_OPTS) as ydl:
            # Resolve URL directly from SoundCloud API track ID
            sc_url = f"https://api.soundcloud.com/tracks/{track_id}"
            entry = ydl.extract_info(sc_url, download=False)
            url = entry.get('url')
            if url:
                set_cached_url(track_id, url)
